...
const DeletePatient = ({ appointment, onClose, onConfirm }) => {
  const [isDeleting, setIsDeleting] = useState(false);

  // Safely extract patient info
  const patientName = appointment?.patient || "this patient";
  const patientId = appointment?.patientId || appointment?.id || "unknown";

  const handleDelete = async () => {
    if (!patientId || patientId === "unknown") {
      errorToast("Invalid patient ID");
      return;
    }
    ...
    try {
      ...
      // Success toast
      successToast(`Patient "${patientName}" deleted successfully!`);
      ...
      <div className="w-[505px] rounded-[20px] p-[1px] backdrop-blur-md shadow-[0px_0px_4px_0px_#FFFFFF1F] bg-gradient-to-r from-green-400/70 via-gray-300/30 to-green-400/70">
        ...
        >
          ...
          <div className="flex justify-center gap-3 mt-8 relative z-10">
            ...
            <button
              onClick={onConfirm}
              className="w-[144px] h-[34px] rounded-[8px] py-2 px-3 border-b-[2px] border-red-600 bg-gradient-to-r from-[#FF4D4D] via-[#D32F2F] to-[#B30000] shadow text-white font-medium text-[14px] leading-[16px] hover:scale-105 transition flex items-center justify-center gap-2"
            >
              <Trash2 size={16} />
              Delete
            </button>
